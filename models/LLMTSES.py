...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        if self.task_name == 'long_term_forecast':
            ...
            #self.out_layer = nn.Linear(int(configs.d_model / (self.patch_num + configs.prompt_length)),configs.pred_len)
            self.out_layer = nn.Linear(configs.d_model * (self.patch_num + configs.prompt_length), configs.pred_len)
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        outputs = self.out_layer(last_embedding.reshape(B*M, -1))
        #utputs = self.out_layer(last_embedding.reshape(B * M * 3, -1))
